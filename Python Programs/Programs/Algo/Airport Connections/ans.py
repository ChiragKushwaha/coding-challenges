...
def airportConnections(airport, routes, startingAirport):
    airportGraph = crateAirportGraph(airport, routes)
    unreachableAirportNodes = getUnreachableAirportNodes(airportGraph, airport, startingAirport)
    markUnreachableConnections(airportGraph, unreachableAirportNodes)
    return getMinNumberOfNewConnections(airportGraph, unreachableAirportNodes)


# O(a + r) time | O(a + r) space
def crateAirportGraph(airports, routes):
    airportGraph = {}
...
def markUnreachableConnections(airportGraph, unreachableAirportNodes):
    for airportNode in unreachableAirportNodes:
        ...
        depthFirstAddUnreachableConnection(airportGraph, airport, unreachableConnections, {})
        airportNode.unreachableConnections = unreachableConnections


def depthFirstAddUnreachableConnection(airportGraph, airport, unreachableConnections, visitedAirports):
